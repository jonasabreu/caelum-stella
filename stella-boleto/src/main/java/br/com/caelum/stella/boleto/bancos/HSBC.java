...
public class HSBC implements Banco {
	...
	public String getCodigoDoDocumentoFinalComDigitosVerificadores(Boleto boleto) {
		return String.format("%s%d%d%d", boleto.getNoDocumento(), getPrimeiroDigitoVerificador(boleto),
	...
	private int getTipoIdentificador(Boleto boleto) {
		int tipo = 4;
		if (boleto.getDatas().getVencimento() == null || boleto.getFatorVencimento() == null
				|| boleto.getFatorVencimento().trim().equals("")) {
			// tipo = 5;
	...
	public String getDataFormatoJuliano(Calendar vencimento, int tipo) {
		...
		if (vencimento.before(dataLimite)) {
			...
			if (tipo == 4) {
				/*
				 * http://java.sun.com/j2se/1.4.2/docs/api/java/util/GregorianCalendar
				 * .html
				 * 
				 ...
				 * 
				 * A linha abaixo n�o funcionar� corretamente apenas no ano de
				 * 2100.
				 */
	...
	public String getNossoNumeroDoEmissorFormatado(Emissor emissor) {
		...
		return format.format(new Long(emissor.getNossoNumero()));
	}
